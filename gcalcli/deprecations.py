...
CAMELS = {'--configFolder': '--config-folder', '--defaultCalendar': '--default-calendar'}
...
def warn_deprecated_opt(option_string):
    ...
    suggestion = (
        suggestion.format(CAMELS[option_string])
        if option_string in CAMELS
        else suggestion.format(option_string.replace('_', '-'))
    )

    msg = (
        'WARNING: {} has been deprecated and will be removed in a future '
        'release.\n' + suggestion
    )
...
BASE_OPTS = {
    'program': {'type': str, 'help': argparse.SUPPRESS, 'action': DeprecatedStore},
    'color': {
        'type': valid_color_name,
        'help': argparse.SUPPRESS,
        'action': DeprecatedStore,
    },
    'remind': {'help': argparse.SUPPRESS, 'action': DeprecatedStoreTrue},
}
...
OPTIONS: dict[str, dict[str, Any]] = {
    ...
    'remind': {'--default_reminders': {'action': DeprecatedStoreTrue, 'default': False}},
}
    ...
    if callable(getter_func):
        ...
        def wrapped(*args, **kwargs):
            ...
            for arg, options in OPTIONS[name].items():
                parser.add_argument(arg, default=options['default'], **BASE_OPTS[name])
            return parser
