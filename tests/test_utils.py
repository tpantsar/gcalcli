...
def test_get_parsed_timedelta_from_str():
    assert utils.get_timedelta_from_str('3.5h') == timedelta(hours=3, minutes=30)
    ...
    assert utils.get_timedelta_from_str('1h1m') == timedelta(hours=1, minutes=1)
    assert utils.get_timedelta_from_str('1:10') == timedelta(hours=1, minutes=10)
    ...
    assert utils.get_timedelta_from_str(
        '2 days 1 hour 2 minutes 40 seconds'
    ) == timedelta(days=2, hours=1, minutes=2, seconds=40)
...
def test_get_times_from_duration():
    ...
    assert (begin_1970, next_day) == utils.get_times_from_duration(
    ...
    with pytest.raises(ValueError):
        utils.get_times_from_duration(begin_1970_midnight, duration='not a duration')
